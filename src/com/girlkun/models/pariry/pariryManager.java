            ...
            if (currentPariry.getResult() == true) {
                if (currentPariry.getPlayersOdd() != null && currentPariry.getPlayersOdd().size() > 0) {
                    for (Player pl : currentPariry.getPlayersOdd()) {
                        ...
                        double winAmount1 = pl.cuoc * 1.8;
                        ChatGlobalService.gI().chat(pl,pl.name + " đã thắng " + Util.format(winAmount1) + " thỏi vàng");
                        Service.gI().sendThongBaoOK(pl,"Bạn đã thắng " + Util.format(winAmount1) + " thỏi vàng");
                        pl.cuoc = 0;
                ...
                if (currentPariry.getPlayersEven() != null && currentPariry.getPlayersEven().size() > 0) {
                    for (Player pl : currentPariry.getPlayersEven()) {
                        ...
                        addPlayerToNewSession(pl, true, false);
                         double winAmount = pl.cuoc1 * 1.8;
                        ChatGlobalService.gI().chat(pl,pl.name + " đã thắng " + Util.format(winAmount) + " thỏi vàng");
                        Service.gI().sendThongBaoOK(pl, "Bạn đã thắng " + Util.format(winAmount) + " thỏi vàng");
                        pl.cuoc1 = 0;
                        pariryManager.gI().goldChan = 0;
                        pariryManager.gI().goldLe = 0;
                    }
