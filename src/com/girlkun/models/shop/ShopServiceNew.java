...
public class ShopServiceNew {
    ...
    private boolean subIemByItemShop(Player pl, ItemShop itemShop) {
        ...
        switch (itS.template.id) {
            ...
            case 1004:
                if (itemShop.tabShop.shop.tagName.equals("PGG")) {

                    for (Item i : pl.inventory.itemsBag) {
                        if (i.template != null) {
                                if (InventoryServiceNew.gI().findItemBag(pl, 459) == null || !InventoryServiceNew.gI().findItemBag(pl, 459).isNotNullItem()) {
                                    Service.getInstance().sendThongBao(pl, "Không có phiếu giảm giá");
                                    return false;
                                } else if (InventoryServiceNew.gI().findItemBag(pl, itSpec) == null || !InventoryServiceNew.gI().findItemBag(pl, itSpec).isNotNullItem()) {
                                    ...
                                    InventoryServiceNew.gI().subQuantityItemsBag(pl, InventoryServiceNew.gI().findItemBag(pl, 459), 1);
                                    InventoryServiceNew.gI().subQuantityItemsBag(pl, InventoryServiceNew.gI().findItemBag(pl, itSpec), buySpec);
                                    return true;
                                }
                        }
