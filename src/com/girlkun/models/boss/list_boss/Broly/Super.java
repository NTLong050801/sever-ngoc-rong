...
public class Super extends Boss {
    ...
    public void active() {
        ...
        if (this.nPoint.hpMax < 40707770){
                    this.nPoint.hpMax+=this.nPoint.hpMax/20;}
            if(this.nPoint.hpMax > 40707770){
                    this.nPoint.hpMax=40707770;}}}
        super.active();
...
public float injured(Player plAtt, float damage, boolean piercing, boolean isMobAttack) {
    if (plAtt != null) {
        switch (plAtt.playerSkill.skillSelect.template.id) {
            ...
            case Skill.THOI_MIEN: {
                // Gây sát thương bằng 1% máu tối đa
                damage = this.nPoint.hpMax / 100;
                float result = super.injured(plAtt, damage, !piercing, isMobAttack);

                // Sau khi bị thương, kiểm tra nếu máu tụt xuống < 1 thì chết
                if (this.nPoint.hp < 1) {
                    this.setDie(plAtt);
                    die(this);
                }
            ...
            case Skill.QUA_CAU_KENH_KHI: {
                // Gây sát thương bằng 10% máu hiện tại
                damage = this.nPoint.hp / 10;
                float result = super.injured(plAtt, damage, !piercing, isMobAttack);
    ...
    // Mặc định xử lý bình thường nếu không thuộc skill đặc biệt
