...
boolean going;
    ...
    private String getTextStatus(byte status) {
        switch (status) {
            case FOLLOW:
                return "Con sẽ đi theo sư phụ!";
            case PROTECT:
                return "Đừng để ta phải nổi giận!";
            case ATTACK:
                return "Sư phụ cứ để con lo cho";
            case GOHOME:
                return "Tạm biệt sư phụ";
            default:
                return "";
        }
    }
...
public void gohome(Player plAtt) {

        if (plAtt != null && plAtt != this && plAtt != this.master && isTargerDe(plAtt)) {
            this.playerAttack = plAtt;
            if (System.currentTimeMillis() - lastTimegoing > 0) {
                if (this.playerAttack.isPet) {
                    this.chat( "Mi làm ta nổi giận rồi đó!: " + playerAttack.name
                            .replace("$", ""));
                    ...
                    this.chat(
                            "Mi làm ta nổi giận rồi đó!: " + playerAttack.name
                                    .replace("$", ""));
                }
            }
            lastTimegoing = System.currentTimeMillis();
