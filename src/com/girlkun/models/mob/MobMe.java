...
public final class MobMe extends Mob {
    ...
    public MobMe(Player player) {
        ...
        this.point.maxHp = SkillUtil.getHPMobMe(Util.LeHaiGH(player.nPoint.hpMax), level);
        this.point.dame = SkillUtil.getHPMobMe(Util.LeHaiGH(player.nPoint.getDameAttack(false)), level);
        ...
        try {
            if (pl != null) {
                if (pl.nPoint.hp > this.point.dame && pl.nPoint.hp > pl.nPoint.hpMax * 0.05) {
                    float dameHit = pl.injured(null, this.point.dame, true, true);
                    msg = new Message(-95);
                    msg.writer().writeByte(2);

                    msg.writer().writeInt(this.id);
                    msg.writer().writeInt((int) pl.id);
                    msg.writer().writeInt(Util.LeHaiGH(dameHit));
