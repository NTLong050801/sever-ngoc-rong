        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN: //kaioken
                ...
                if (player.nPoint.hp <= hpUse) {
                    ...
                    player.nPoint.setHp(Util.LeHaiGH(player.nPoint.mp - hpUse));
...
//                    mobTarget.attackMob(player, false, false);
        ...
        switch (player.playerSkill.skillSelect.template.id) {
             case Skill.THAI_DUONG_HA_SAN:
                ...
                if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                    ...
                    for (Player pl : playersMap) {
                        if (pl != null && !player.equals(pl)&&((player.cFlag!=0&&(player.cFlag!=pl.cFlag&&pl.cFlag!=0))||(pl.cFlag==8&&player.cFlag!=0)||(pl.cFlag!=0&&player.cFlag==8)||(pl.typePk==5)|| (player.typePk == pl.typePk && player.typePk != (byte) 0 && pl.typePk != (byte) 0)) && !player.nPoint.khangTDHS) {
                    ...
                    if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                        ...
                        for (Player pl : playersMap) {
                            if (!pl.isBoss && pl.gender != ConstPlayer.NAMEC
                                ...
                                pl.nPoint.calPoint();
                                pl.nPoint.setHp(Util.LeHaiGH((pl.nPoint.hp + (pl.nPoint.hp * tileHP / 100))));
                        ...
                        player.nPoint.setHp(Util.LeHaiGH((player.nPoint.hp + (player.nPoint.hp * tileHP / 100))));
                        Service.getInstance().point(player);
                        Service.getInstance().Send_Info_NV(player);
            ...
            case Skill.TU_SAT:
                if (!player.playerSkill.prepareTuSat) {
                    ...
                    int dame = Util.LeHaiGH(player.nPoint.hpMax);
                    for (Mob mob : player.zone.mobs) {
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.TRI_THUONG:
                ...
                if (canHsPlayer(player, plTarget)) {
                    ...
                    long hpHoiMe = Util.LeHaiGH(player.nPoint.hp * percentTriThuong / 100);;
    ...
    private void phanSatThuong(Player plAtt, Player plTarget, float dame) {
        int percentPST = plTarget.nPoint.tlPST;
        if (percentPST != 0) {
            float damePST = dame * percentPST / 100;
            Message msg;
            try {
                msg = new Message(56);
                msg.writer().writeInt((int) plAtt.id);
                if (damePST >= plAtt.nPoint.hp) {
                    damePST = Util.LeHaiGH(plAtt.nPoint.hp - 1);
                }
                damePST = plAtt.injured(null, damePST, true, false);
                msg.writer().writeInt(Util.LeHaiGH(plAtt.nPoint.hp));
                msg.writer().writeInt(Util.LeHaiGH(damePST));
    ...
    private void hutHPMP(Player player, float dame, boolean attackMob) {
        int tiLeHutHp = player.nPoint.getTileHutHp(attackMob);
        int tiLeHutMp = player.nPoint.getTiLeHutMp();
        float hpHoi = dame * tiLeHutHp / 100;
        float mpHoi = dame * tiLeHutMp / 100;
        if (hpHoi > 0 || mpHoi > 0) {
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        float dameHit = plInjure.injured(plAtt, miss ? 0 : plAtt.nPoint.getDameAttack(false), false, false);
        ...
        try {
            ...
            if (dameHit > 2123456789){
                ...
                dameHit = 2123456000;
            msg.writer().writeInt(Util.LeHaiGH(dameHit)); //dame ăn
            msg.writer().writeInt(Util.LeHaiGH(dameHit)); //dame ăn
                    }
            ...
            if (typeSkill != 1) {
                ...
                msg.writer().writeByte(typeSkill == 2 ? 0 : 1); //read continue
                msg.writer().writeByte(0); //type skill
                msg.writer().writeInt(Util.LeHaiGH(dameHit)); //dame Äƒn
                msg.writer().writeBoolean(plInjure.isDie()); //is die
                msg.writer().writeBoolean(plAtt.nPoint.isCrit); //crit
        ...
        if (plInjure != null && !plInjure.isDie() && plInjure.nPoint.hp > Integer.MAX_VALUE) {
            Service.gI().chat(plInjure,  "|7|"  + plInjure.name + "\bHP: " + Util.getFormatNumber(plInjure.nPoint.hp) +"\b </>");
        }
        if (plInjure != null && !plInjure.isDie() && plInjure.nPoint.dame > Integer.MAX_VALUE) {
            Service.gI().chat(plAtt, "Đame : " + plAtt.name +"\b" +  Util.numberToMoney(dameHit));
        }} catch (Exception e) {
            Logger.logException(SkillService.class, e);
        }
    }
    ...
    private void playerAttackMob(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) {
        if (!mob.isDie()) {
            ...
            if(dameHit<0)
            {
                dameHit = 20000000000L;
            }
            hutHPMP(plAtt, dameHit, true);
            sendPlayerAttackMob(plAtt, mob);
            mob.injured(plAtt, Util.LeHaiGH(dameHit), dieWhenHpFull);
        }
    }
