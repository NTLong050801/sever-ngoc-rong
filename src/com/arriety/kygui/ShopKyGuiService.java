...
//                msg.writer().writeByte(0);
    ...
    public void StartupItemToTop(Player pl) {
        if (!SubThoiVang(pl, 100)) {
            Service.gI().sendThongBao(pl, "Bạn cần có ít nhất 100 thỏi vàng đưa vật phẩm lên trang đầu");
            return;
        }
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            if (!SubThoiVang(pl, 5)) {
                Service.gI().sendThongBao(pl, "Bạn cần có ít nhất 5 thỏi vàng để làm phí đăng bán");
                return;
            }

            Item it = ItemService.gI().copyItem(pl.inventory.itemsBag.get(id));
            for (Item.ItemOption daubuoi : it.itemOptions) {
                if (daubuoi.optionTemplate.id == 30 || daubuoi.optionTemplate.id == 93) {
